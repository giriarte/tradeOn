...
class TestNGreenCandles(unittest.TestCase):
    ...
    def test_successful_match(self):
        ...
        params = {'n_candles': 3, 'n_candles_operation': 1}
    ...
    def test_edge_case_n_equals_data_length(self):
        ...
        params = {'n_candles': 4, 'n_candles_operation': 1}
        result = self.indicator.evaluate(self.df_match, params)
    ...
    def test_edge_case_n_equals_one(self):
        ...
        params = {'n_candles': 1, 'n_candles_operation': 1}
    ...
    def test_value_error_n_missing(self):
        """Test case for missing 'n_candles' parameter."""
        with self.assertRaisesRegex(ValueError, "Parameters must be provided and contain the key 'n_candles'"):
            self.indicator.evaluate(self.df_match, params={})

    def test_value_error_n_invalid(self):
        """Test case for non-positive or non-integer 'n_candles' parameter."""
        with self.assertRaisesRegex(ValueError, "'n_candles' must be a positive integer."):
            self.indicator.evaluate(self.df_match, params={'n_candles': 0, 'n_candles_operation': 1})
        
        with self.assertRaisesRegex(ValueError, "'n_candles' must be a positive integer."):
            self.indicator.evaluate(self.df_match, params={'n_candles': -2, 'n_candles_operation': 1})
            
        with self.assertRaisesRegex(ValueError, "'n_candles' must be a positive integer."):
            self.indicator.evaluate(self.df_match, params={'n_candles': 2.5, 'n_candles_operation': 1})
