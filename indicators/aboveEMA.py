...
class AboveEMA(Indicator):
    """
    Checks if the current price is strictly above a calculated EMA.
    
    Parameters:
    - 'ema_length': (int) The period for the EMA calculation (e.g., 20, 50, 200).
    - 'operation_type': (int) Signal to return if price > EMA (default SIGNAL_BUY).
    """
    ...
    def evaluate(self, data: pd.DataFrame, params: t.Optional[t.Dict[str, t.Any]] = None) -> int:
        ...
        # --- 4. Logic Execution ---
        # Trigger if the price is currently above the average
